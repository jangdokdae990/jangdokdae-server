"""EmbeddingClusterer — 임베딩·중복제거·클러스터링·이슈 선정 단계 조립(설계 05 §8).

흐름: (embed_news ∥ embed_chunks) → deduplicate → cluster → score_and_select.
분기·LLM 추론이 없는 정적 순차라 Airflow Task로 실행된다(설계 01 §2). 단계끼리 직접
호출하지 않고 공유 DB 상태 컬럼(embedding·is_duplicate·news_cluster)으로만 핸드오프한다.

세션 주의: 두 임베딩은 독립적이라 asyncio.gather로 병렬 실행하되(05 §8.3), AsyncSession은
동시 사용이 안전하지 않으므로 각자 **독립 세션**을 연다. 이후 중복제거·클러스터링·적재는
넘겨받은 db 하나로 순차 처리한다.
"""
...
class EmbeddingClustererState(TypedDict):
    """단계 실행 결과 요약 — 카운트와 실패 신호만 담는다(데이터는 DB 핸드오프, 설계 01 §2)."""
...
class EmbeddingClusterer:
    ...
    def __init__(
        ...
        # 클라이언트를 명시 주입하면 두 임베더가 공유한다(HuggingFace 등 무거운 백엔드).
        # 미주입(None)이면 각 임베더가 작업이 있을 때만 lazy 생성한다 — no-op 실행에서
        # 백엔드 구축 비용이 0이고, 프로덕션 Vertex 백엔드는 래퍼 생성이 저렴해 공유 불필요.
    ...
    async def run(self, db: AsyncSession) -> EmbeddingClustererState:
        ...
        # 단계 간 처리 범위가 어긋나면 dedup 안 된 행이 클러스터에 섞인다(05 §4.2·§6).
    ...
    async def _cluster_and_select(self, db: AsyncSession, since: datetime) -> tuple[int, list[int]]:
        ...
        since: 수집 시각 하한(run()이 dedup과 공유하는 창) — 경계가 없으면 분석 게이트
        (is_analyzed)가 닫히기 전의 백로그 전체가 매일 재클러스터링된다(05 §6).
        """
        ...
        labels = promote_singletons(labels)  # noise(-1)도 size-1 클러스터로 보존(§5.4)
    ...
    def _score_clusters(
        ...
        """각 클러스터를 중심 근접순 정렬 후 복합 중요도로 스코어링한다(§5.8·§6.1).

        Sentiment·Entity·prev_cluster_size는 상류·이력이 아직 없어 0이다(MVP, → 05 §11).
        볼륨 지배 정렬이라 카카오 RUBICS(볼륨=주요 이슈)와 정합한다.
        """
