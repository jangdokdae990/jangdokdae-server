...
> **작성자** Kim minkyoung · **작성일** 2026-06-08 (2026-06-18 재설계 개정)
>
...
| 한계 | 무슨 문제 | 전환 신호 |
|------|----------|----------|
| **단일 호스트 SPOF** | 호스트가 죽으면 스케줄러·DB 전부 정지 | 가용성 SLA 필요 시 |
| **수동 재시작** | 다운 시 자동 복구 없음(`restart: always`로 완화하나 호스트 장애엔 무력) | 무중단 운영 요구 |
| **스케일아웃 불가** | LocalExecutor는 단일 머신 — 수직 확장만 | 동시 Task가 VM 한 대 초과 |
| **백필·대량 재처리 부담** | 큰 backfill을 단일 머신이 직렬 처리 | 과거 대량 재처리 상시화 |
