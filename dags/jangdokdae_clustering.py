...
근접중복 제거 + 최근 14일 윈도우 전체 재클러스터링 + cluster id 승계를 수행한다(고정 cron 없음).
analyze Task(L2)는 클러스터링 출력 변화 트리거로 후속 연결(06 TODO).
...
with DAG(
    ...
    # TODO: analyze Task — NewsAnalysisAgent 구현 후 cluster >> analyze 연결(06)
